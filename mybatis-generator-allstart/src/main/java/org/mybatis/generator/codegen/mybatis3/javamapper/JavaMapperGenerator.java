...
public class JavaMapperGenerator extends AbstractJavaClientGenerator {
	...
	public List<CompilationUnit> getCompilationUnits() {
		...
		// add superinterface and genericType on this interface
		String basemapper = context.getJavaClientGeneratorConfiguration()
				.getProperty(PropertyRegistry.JAVA_TYPE_SUPER_MAPPER);
		if (!StringUtils.isBlank(basemapper)) {
			...
			FullyQualifiedJavaType superInterface = FullyQualifiedJavaType.getNewBaseMapper(basemapper);
		...
		if (ConstKey.isPrint) {
			System.err.println("\n");
			System.err.println("Java mapper start ...");
			System.err.println("\n");
	...
	protected void addQueryAllNoPageMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateQueryListNoPage()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new QueryAllNoPageMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}

	protected void addQueryAll(Interface interfaze) {
		if (introspectedTable.getRules().generateQueryList()) {
			AbstractJavaMapperMethodGenerator methodGenerator = new QueryAllOnPageMethodGenerator();
			initializeAndExecuteGenerator(methodGenerator, interfaze);
		}
	}

	protected void addCountAllMethod(Interface interfaze) {
		if (introspectedTable.getRules().generateCountList()) {
